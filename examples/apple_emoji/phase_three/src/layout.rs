...
//! inline box sized to its **Noto Color Emoji** advance width (Phase Three).
...
use crate::noto_advances::{NOTO_ADVANCES, NOTO_UPEM};

/// Look up the Noto Color Emoji advance for `cp`, in px at `font_size`. Binary-searches
/// the generated `NOTO_ADVANCES` table (sorted by codepoint).
pub(crate) fn noto_advance_px(cp: u32, font_size: f32) -> Option<f32> {
    NOTO_ADVANCES
        .binary_search_by_key(&cp, |&(c, _)| c)
        .ok()
        .map(|i| NOTO_ADVANCES[i].1 as f32 / f32::from(NOTO_UPEM) * font_size)
}
