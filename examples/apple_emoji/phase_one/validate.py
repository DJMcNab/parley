...
"""Dependency-free ChromeDriver validation harness for Phase One.

Serves the trunk `dist/` build over HTTP, drives Chrome for Testing via
ChromeDriver's W3C WebDriver protocol (using only the stdlib `urllib`, no
`selenium`), waits for the page to report "done", scrapes the metrics/verdict
JSON and takes a screenshot.
...
def run_session(base, session_id, http_port):
    ...
    data_result = wd_request(
        ...
        f"/session/{session_id}/execute/sync",
        {
            "script": "return document.getElementById('data-phase-one').textContent;",
            "args": [],
        },
    )
    ...
    if data_json_text:
        try:
            ...
            color_ok = metrics.get("colored_pixel_count", 0) > 0
    ...
    return status_text, metrics, color_ok, png_bytes
...
def main():
    ...
    try:
        status_text, metrics, color_ok, png_bytes = attempt(cd_base, http_port, headless=True)
        mode = "headless"
        if not color_ok:
            print("headless color check failed or inconclusive; retrying headed…")
            status_text, metrics, color_ok, png_bytes = attempt(cd_base, http_port, headless=False)
            mode = "headed"
    finally:
    ...
    screenshot_path = ARTIFACTS_DIR / "phase_one.png"
    ...
    passed = bool(metrics and metrics.get("pass"))
    print()
